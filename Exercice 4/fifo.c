...
fifo* FF_append(fifo *ff, messages *message, int id, int length, double arrival_time, struct sockaddr *destination)
{
	...
	{
		newElt->_message = malloc(sizeof(struct message_s));
		strncpy(newElt->_message->buf, message->buf, length);
		newElt->_id = id;
		newElt->_length = sizeof(struct message_s);
...
int FF_head(fifo *ff, messages *message, int *id, int *length, double *arrival_time, struct sockaddr **destination)
{
	if(FF_isEmpty(ff))
	{
	...
	{
		fifo *head = ff->_next;
		strcpy(message->buf,head->_message->buf);
		*length = head->_length;
		*id = head->_id;
...
void FF_print(fifo *ff, messages *message)
{
	...
	else
	{
		...
		while(!finished)
		{
			...
			fprintf(stderr,"\x1b[34mDatagram number %d : \x1b[37m\t arrived at time %f, Destination host : %s\t port %s \t message (%d) : %s\n",
					nb_elems, f->_arrival_time, host, port, f->_length, f->_message->buf);
